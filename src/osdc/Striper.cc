...
/**
 * 将文件的逻辑偏移量转换为对象存储系统中的对象和偏移量。
 *
 * @param cct CephContext 对象，用于日志记录和配置。
 * @param layout 文件布局信息。
 * @param offset 文件的逻辑偏移量。
 * @param len 要转换的长度。
 * @param trunc_size 截断大小。
 * @param buffer_offset 缓冲区偏移量。
 * @param object_extents 输出参数，包含转换后的对象和偏移量信息。
 */
void Striper::file_to_extents(
  ...
  while (left > 0) {
    ...
    // map range into object // 将范围映射到对象中
