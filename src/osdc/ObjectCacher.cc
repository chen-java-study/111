...
/**
 * 执行一个文件写入操作，将数据写入到对象存储系统中。
 *
 * @param wr 指向要写入的对象的指针。
 * @param oset 指向要写入的对象集的指针。
 * @param onfreespace 当有足够空间时调用的回调函数。
 * @param parent_trace 父跟踪对象，用于跟踪操作的执行过程。
 * @param block_writes_upfront 是否在前端阻塞写入操作。
 * @return 操作的结果状态。
 */
			 ...
			 bool block_writes_upfront)
{
  ...
  // 初始化已写入字节数和在刷新中写入的字节数
  uint64_t bytes_written = 0;
  uint64_t bytes_written_in_flush = 0;
  // 检查是否设置了 LIBRADOS_OP_FLAG_FADVISE_DONTNEED 标志
  bool dontneed = wr->fadvise_flags & LIBRADOS_OP_FLAG_FADVISE_DONTNEED;
  // 检查是否设置了 LIBRADOS_OP_FLAG_FADVISE_NOCACHE 标志
...
// 累计已写入的字节数
    ...
    if (bh->is_tx()) {
         // 如果是事务性写入，则累计在刷新中写入的字节数
      bytes_written_in_flush += ex_it->lengt
      h;
    }
    // 调整缓冲区指针，即将数据复制到缓存中
    // 这是针对单个 ObjectExtent 的操作，因此我们知道：
    // - 有一个连续的 bh
    // - 缓冲区片段可能不连续（实际上几乎肯定不连续）
    // 注意：我假设条带化是单调的...没有向后跳跃，永远！
    ...
    for (vector<pair<uint64_t, uint64_t> >::iterator f_it
	 ...
	 ++f_it) {
      ldout(cct, 10) << "writex writing " << f_it->first << "~"
		     ...
		     << dendl;
             // 计算缓冲区片段在 BufferHead 中的偏移量
      uint64_t bhoff = opos - bh->start();
       // 断言片段长度不超过 BufferHead 的剩余长度
      ceph_assert(f_it->second <= bh->length() - bhoff);

      // get the frag we're mapping in // 获取要映射的片段
      bufferlist frag;
      frag.substr_of(wr->bl, f_it->first, f_it->second);

      // keep anything left of bhoff // 处理片段在 BufferHead 中的位置
      if (!bhoff)
        bh->bl.swap(frag);
      else
...
// 更新偏移量
    ...
    // ok, now bh is dirty.// 标记 BufferHead 为脏
    mark_dirty(bh);
    // 如果设置了 dontneed 标志，则设置 BufferHead 为不需要
    if (dontneed)
      bh->set_dontneed(true);
       // 如果设置了 nocache 标志且 BufferHead 缺失，则设置为不缓存
    else if (nocache && missing)
      bh->set_nocache(true);
      // 否则，标记 BufferHead 为已访问
    else
      touch_bh(bh);
...
// 如果设置了最大脏数据限制，并且写操作没有强制写入磁盘标志
  ...
  // start writeback anyway?// 检查是否需要启动写入缓存的刷新
