...
public:
  ...
  void sg_write_trunc(std::vector<ObjectExtent>& extents, const SnapContext& snapc,
    ...
    if (extents.size() == 1) {// 如果只有一个片段，则直接调用 write_trunc 函数进行写入
      ...
      C_GatherBuilder gcom(cct, oncommit);// 如果有多个片段，则使用 C_GatherBuilder 进行分散写入
      auto it = bl.cbegin();
      for (auto p = extents.begin(); p != extents.end(); ++p) { // 遍历每个片段
	ceph::buffer::list cur;
    // 遍历当前片段的缓冲区范围
	for (auto bit = p->buffer_extents.begin();
	     bit != p->buffer_extents.end();
	     ++bit) {
             // 如果迭代器的偏移量不等于当前缓冲区范围的起始偏移量
	  if (it.get_off() != bit->first) {
        // 则将迭代器移动到当前缓冲区范围的起始偏移量
	    it.seek(bit->first);
	  }
      // 将数据从迭代器复制到当前片段的缓冲区列表中
	  it.copy(bit->second, cur);
	}
	ceph_assert(cur.length() == p->length);
    // 调用 write_trunc 函数，将数据写入到当前片段中
