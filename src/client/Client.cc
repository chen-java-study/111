 ...
 * 向文件写入数据
 * @param f 文件句柄
 * @param offset 写入数据的起始偏移量
 * @param size 要写入的数据字节数
 * @param buf 指向数据缓冲区的指针
 * @param iov 指向 iovec 数组的指针，用于指定多个数据缓冲区
 * @param iovcnt iovec 数组的长度
 * @param onfinish 写入完成后的回调函数
 * @param do_fsync 是否在写入完成后进行文件同步
 * @param syncdataonly 是否只同步数据而不同步元数据
 * @return 返回写入的字节数，失败则返回负值
 */
int64_t Client::_write(Fh *f, int64_t offset, uint64_t size, const char *buf,
	                const struct iovec *iov, int iovcnt, Context *onfinish,
	                bool do_fsync, bool syncdataonly)
{
    // 确保当前线程持有 client_lock 互斥锁
  ceph_assert(ceph_mutex_is_locked_by_me(client_lock));

  uint64_t fpos = 0;// 初始化文件位置偏移量
  Inode *in = f->inode.get(); // 获取文件对应的 Inode 指针
  std::unique_ptr<C_SaferCond> onuninline = nullptr;// 初始化一个用于处理异步操作的条件变量
  CWF_iofinish *cwf_iofinish = NULL;  // 初始化一个用于处理 I/O 完成的回调函数
  C_SaferCond *cond_iofinish = NULL;// 初始化一个用于处理 I/O 完成的条件变量
    // 检查写入的字节数是否小于 1，如果是，则不支持零字节写入
  if (size < 1) { // zero bytes write is not supported by osd
    return -CEPHFS_EINVAL;
  }
  // 检查写入的字节数是否超过了文件系统的最大文件大小限制
...
// 确保文件的快照 ID 为 CEPH_NOSNAP，表示没有快照
  ...
  // 如果偏移量小于 0，则需要调整文件指针位置
  if (offset < 0) {
    lock_fh_pos(f);// 锁定文件的位置偏移量
    /*
     ...
     */
     // 如果文件是以追加模式打开的
    if (f->flags & O_APPEND) {
      auto r = _lseek(f, 0, SEEK_END);// 尝试将文件指针移动到文件末尾
      if (r < 0) {// 如果移动失败
        unlock_fh_pos(f); // 解锁文件的位置偏移量
        return r;
      }
    }
    offset = f->pos;// 将文件的当前位置设置为偏移量
    fpos = offset+size;// 计算文件的新位置
    unlock_fh_pos(f);// 解锁文件的位置偏移量
  }
  ...
  if (endoff > in->size && is_quota_bytes_exceeded(in, endoff - in->size,
						   f->actor_perms)) {
    return -CEPHFS_EDQUOT; // 返回错误码 -CEPHFS_EDQUOT，表示超出配额
  }
  ...
  // time it.
  utime_t start = ceph_clock_now();// 记录开始时间
     // 如果文件的内联版本为 0，则需要获取内联数据的属性
  if (in->inline_version == 0) {
    int r = _getattr(in, CEPH_STAT_CAP_INLINE_DATA, f->actor_perms, true);// 获取文件的内联数据属性
    if (r < 0)
      return r;
    ceph_assert(in->inline_version > 0);
  }
  ...
  // copy into fresh buffer (since our write may be resub, async)
   // 将数据复制到新的缓冲区中（因为我们的写入操作可能会被重新提交，所以需要异步处理）
  bufferlist bl;
  if (buf) {// 如果有数据缓冲区
    if (size > 0) // 如果数据大小大于 0
      bl.append(buf, size); // 将数据添加到缓冲区中
  } else if (iov){// 如果有 iovec 数组
    for (int i = 0; i < iovcnt; i++) {// 遍历 iovec 数组
      if (iov[i].iov_len > 0) {      // 如果当前 iovec 元素的长度大于 0
        bl.append((const char *)iov[i].iov_base, iov[i].iov_len);// 将数据添加到缓冲区中
      }
    }
  }
// 计算需要的和已经拥有的权限
  int want, have;
  if (f->mode & CEPH_FILE_MODE_LAZY) // 需要文件缓冲和 lazyio 权限
    want = CEPH_CAP_FILE_BUFFER | CEPH_CAP_FILE_LAZYIO;
  else
    want = CEPH_CAP_FILE_BUFFER;// 只需要文件缓冲权限
...
// 增加文件的共享认证权限引用计数
  ...
  if (size > 0) {// 如果数据大小大于 0
    r = clear_suid_sgid(in, f->actor_perms); // 清除文件的 suid 和 sgid 权限
    if (r < 0) {
      put_cap_ref(in, CEPH_CAP_FILE_WR);
      return r;
    }
  }
// 如果文件是以 O_DIRECT 模式打开的
  if (f->flags & O_DIRECT)
    have &= ~(CEPH_CAP_FILE_BUFFER | CEPH_CAP_FILE_LAZYIO);// 清除文件缓冲和 lazyio 权限

  ldout(cct, 10) << " snaprealm " << *in->snaprealm << dendl;
// 初始化一些用于异步 I/O 操作的上下文和完成器
  std::unique_ptr<Context> iofinish = nullptr;
  std::unique_ptr<C_Write_Finisher> cwf = nullptr;
  std::unique_ptr<Context> filer_iofinish = nullptr;
  // 如果文件的内联版本小于 CEPH_INLINE_NONE
  if (in->inline_version < CEPH_INLINE_NONE) {
     // 如果写入的数据大小超过了客户端配置的最大内联大小，或者超过了 CEPH_INLINE_MAX_SIZE，或者没有文件缓冲权限
    if (endoff > cct->_conf->client_max_inline_size ||
        endoff > CEPH_INLINE_MAX_SIZE ||
        !(have & CEPH_CAP_FILE_BUFFER)) {
      onuninline.reset(new C_SaferCond("Client::_write_uninline_data flock"));// 创建一个新的 C_SaferCond 对象，用于在非内联数据写入完成时通知
      uninline_data(in, onuninline.get()); // 启动非内联数据写入操作
    } else {
      get_cap_ref(in, CEPH_CAP_FILE_BUFFER);// 增加文件缓冲权限引用计数

      uint32_t len = in->inline_data.length();// 获取内联数据的长度
...
// 如果有写入完成回调函数
  if (onfinish) {
     cwf_iofinish = new CWF_iofinish(); // 创建一个新的 CWF_iofinish 对象，用于在 I/O 操作完成时通知回调函数
     iofinish.reset(cwf_iofinish);
// 创建一个新的 C_Write_Finisher 对象，用于管理异步写入操作的完成
     cwf.reset(new
